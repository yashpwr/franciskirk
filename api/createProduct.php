...
$query = "SELECT * FROM product_create_data WHERE status IN ('pending') ORDER BY id ASC LIMIT 0,100";
...
if(mysqli_num_rows($result) > 0){
	...
	while($product_import_data = mysqli_fetch_array($result)){
		...
		$log_writer = new \Zend\Log\Writer\Stream(BP . '/api/CreateFiles/log/'.$history_filename.'product.create.log');
		$log_logger = new \Zend\Log\Logger();
		$log_logger->addWriter($log_writer);

		$bad_writer = new \Zend\Log\Writer\Stream(BP . '/api/CreateFiles/log/'.$history_filename.'product.create.bad');
		...
		foreach($productallDataArry as $key => $productallData){
			...
			if(!checkProductExist($productallData['sku'])){
				...
				//if($action == 'create'){
					...
					$query = "UPDATE product_create_data SET status='".$status."',updated_at='".date('Y-m-d H:i:s')."' WHERE id='".$product_import_data['id']."'";
