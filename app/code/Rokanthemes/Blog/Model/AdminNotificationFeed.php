...
class AdminNotificationFeed extends \Magento\AdminNotification\Model\Feed
{
    ...
    /**
     * Get Rokanthemes Modules Info
     *
     * @return $this
     */
    protected function getRokanthemesModules()
    {
        $modules = array();
        foreach($this->_moduleList->getAll() as $moduleName => $module) {
            if ( strpos($moduleName, 'Rokanthemes_') !== false && $this->_moduleManager->isEnabled($moduleName) ) {
    ...
    {
        return $this->_cacheManager->load('Rokanthemes_admin_notifications_lastcheck');
    }
    ...
    {
        $this->_cacheManager->save(time(), 'rokanthemes_admin_notifications_lastcheck');
        return $this;
    }
