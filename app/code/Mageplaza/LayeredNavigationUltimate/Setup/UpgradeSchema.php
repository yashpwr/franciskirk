        ...
        if (version_compare($context->getVersion(), '2.1.0', '<')) {
            if (!$installer->tableExists('layered_product_pages')) {
                $table = $installer->getConnection()->newTable($installer->getTable('layered_product_pages'))
                    ->addColumn('page_id', Table::TYPE_INTEGER, null, ['identity' => true, 'unsigned' => true, 'nullable' => false, 'primary' => true])
                    ...
                    ->addIndex($installer->getIdxName('layered_product_pages', 'route'), 'route', ['type' => AdapterInterface::INDEX_TYPE_UNIQUE])
