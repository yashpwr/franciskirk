    ...
    )
    {
        ...
        if (!$this->customFilter) {
            ...
            if ($stateConfig['new_enable'] || $stateConfig['onsales_enable'] || $stateConfig['stock_enable']) {
                $customFilter[] = $this->objectManager->create($this->filterTypes[self::STATE_FILTER], ['data' => ['position' => $stateConfig['position']], 'layer' => $layer]);
            }

            $ratingConfig = $this->helper->getFilterConfig('rating');
            if (isset($ratingConfig['rating_enable']) && $ratingConfig['rating_enable']) {
                $customFilter[] = $this->objectManager->create($this->filterTypes[self::RATING_FILTER], ['data' => ['position' => $ratingConfig['position']], 'layer' => $layer]);
            }
